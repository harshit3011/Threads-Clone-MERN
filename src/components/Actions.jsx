...
const Actions = ({post: post_}) => {
	...
	const handleReply = async()=>{
		...
		try {
			...
			const data = await res.json();
			if (!res.ok) return showToast("Error", data.error, "error");
			setPost({...post,replies: [...post.replies, data.text]})
	...
	return (
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalFooter>
		  <Button size={'sm'} mr={3} onClick={handleReply} isLoading={isReplying}>
              Reply
            </Button>
          </ModalFooter>
        </ModalContent>
